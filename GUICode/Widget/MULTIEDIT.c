...
*/
static I32 _GetNumChars(MULTIEDIT_OBJ * pObj) {
...
*/
static I32 _GetXSize(MULTIEDIT_HANDLE hObj, const MULTIEDIT_OBJ* pObj) {
  GUI_RECT Rect;
...
*/
static I32 _GetNumCharsInPrompt(const MULTIEDIT_OBJ* pObj, const char GUI_UNI_PTR * pText) {
...
*/
static I32 _WrapGetNumCharsDisp(MULTIEDIT_HANDLE hObj, const MULTIEDIT_OBJ* pObj, const char GUI_UNI_PTR * pText) {
  I32 xSize, r;
...
*/
static I32 _WrapGetNumBytesToNextLine(MULTIEDIT_HANDLE hObj, const MULTIEDIT_OBJ* pObj, const char* pText) {
  I32 xSize, r;
  xSize = _GetXSize(hObj, pObj);
  if (pObj->Flags & MULTIEDIT_SF_PASSWORD) {
    I32 NumChars, NumCharsPrompt;
    NumCharsPrompt = _GetNumCharsInPrompt(pObj, pText);
...
*       _GetCharDistX
*/
static I32 _GetCharDistX(const MULTIEDIT_OBJ* pObj, const char* pText) {
  I32 r;
...
static void _DispString(MULTIEDIT_HANDLE hObj, const MULTIEDIT_OBJ* pObj, const char* pText, GUI_RECT* pRect) {
  ...
  if (pObj->Flags & MULTIEDIT_SF_PASSWORD) {
    I32 x, NumCharsPrompt, NumCharsLeft = 0;
    NumCharsPrompt = _GetNumCharsInPrompt(pObj, pText);
...
static char * _GetpLine(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, unsigned LineNumber) {
  ...
  if ((unsigned)pObj->CacheLineNumber != LineNumber) {
    if (LineNumber > (unsigned)pObj->CacheLineNumber) {
      ...
      I32 OldNumber = pObj->CacheLineNumber;
...
*/
static I32 _GetCursorLine(MULTIEDIT_HANDLE hObj, const MULTIEDIT_OBJ* pObj, const char* pText, I32 CursorPosChar) {
  const char *pCursor;
  const char *pEndLine;
  I32 NumChars, ByteOffsetNewCursor, LineNumber = 0;
...
*       _GetCursorXY
*/
static void _GetCursorXY(MULTIEDIT_HANDLE hObj, /*const*/ MULTIEDIT_OBJ* pObj, I32* px, I32* py) {
  if (pObj->InvalidFlags & INVALID_CURSORXY) {
    I32 CursorLine = 0, x = 0;
    ...
    if (pObj->hText) {
      ...
      if ((pObj->Flags & MULTIEDIT_SF_PASSWORD)) {
        ...
        I32 xSize;
        const char *p;
        I32 NumChars = 0, NumCharsToNextLine;
        xSize = _GetXSize(hObj, pObj);
        p = pLine;
        while (pLine < pCursor) {
          NumChars++;
...
*/
static I32 _GetTextSizeX(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj) {
  if (pObj->InvalidFlags & INVALID_TEXTSIZE) {
    pObj->TextSizeX = 0;
    if (pObj->hText) {
      I32 NumChars, xSizeLine;
      char *pText, *pLine;
      GUI_SetFont(pObj->pFont);
      pText = (char*) GUI_ALLOC_h2p(pObj->hText);
      do {
...
*       _GetNumVisLines
*/
static I32 _GetNumVisLines(MULTIEDIT_HANDLE hObj, const MULTIEDIT_OBJ* pObj) {
  GUI_RECT Rect;
  WM_GetInsideRectExScrollbar(hObj, &Rect);
...
*/
static I32 _GetNumLines(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ * pObj) {
...
*/
static I32 _InvalidateCursorPos(MULTIEDIT_OBJ * pObj) {
  I32 Value;
...
*       _SetFlag
*/
static void _SetFlag(MULTIEDIT_HANDLE hObj, I32 OnOff, U8 Flag) {
  if (hObj) {
    MULTIEDIT_OBJ * pObj;
    WM_LOCK();
    pObj = MULTIEDIT_H2P(hObj);
...
*/
static I32 _CalcNextValidCursorPos(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, I32 CursorPosChar, I32 * pCursorPosByte, I32 * pCursorLine) {
  if (pObj->hText) {
    char *pNextLine, *pCursor, *pText;
    I32 CursorLine, NumChars, CursorPosByte;
    pText    = (char*) GUI_ALLOC_h2p(pObj->hText);
...
*/
static void _SetCursorPos(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, I32 CursorPosChar) {
  I32 CursorPosByte, CursorLine;
...
*       _SetWrapMode
*/
static I32 _SetWrapMode(MULTIEDIT_HANDLE hObj, GUI_WRAPMODE WrapMode) {
  I32 r;
  r = 0;
  if (hObj) {
    MULTIEDIT_OBJ * pObj;
...
*/
static void _SetCursorXY(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, I32 x, I32 y) {
  I32 CursorPosChar = 0;
  if ((x < 0) || (y < 0)) {
    return;
  }
...
*/
static I32 _IsOverwriteAtThisChar(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj) {
  I32 r = 0;
  if (pObj->hText && !(pObj->Flags & MULTIEDIT_CF_INSERT)) {
    const char *pText;
    I32 CurPos, Line1, Line2;
    U16 Char;
...
*/
static I32 _GetCursorSizeX(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, I32 * pIsRTL) {
  if (_IsOverwriteAtThisChar(hObj, pObj)) {
      ...
      if (pObj->Flags & MULTIEDIT_SF_PASSWORD) {
        ...
        if (pObj->CursorPosChar) {
          I32 NumChars = pObj->CursorPosChar;
          while (--NumChars) {
...
*/
static I32 _IncrementBuffer(MULTIEDIT_OBJ* pObj, unsigned AddBytes) {
  WM_HMEM hNew;
  I32 NewSize;
...
*/
static I32 _IsSpaceInBuffer(MULTIEDIT_OBJ* pObj, I32 BytesNeeded) {
  I32 NumBytes = 0;
  if (pObj->hText) {
    NumBytes = GUI_strlen((char*)GUI_ALLOC_h2p(pObj->hText));
  }
...
*/
static I32 _IsCharsAvailable(MULTIEDIT_OBJ* pObj, I32 CharsNeeded) {
...
static void _DeleteChar(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj) {
  if (pObj->hText) {
    ...
    if (CursorOffset < GUI_strlen(s)) {
      char *pCursor, *pLine, *pEndLine;
      I32 CursorLine, NumChars, NumBytes;
      ...
      GUI_strcpy(pCursor, pCursor + NumBytes);
...
*/
static I32 _InsertChar(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ* pObj, U16 Char) {
...
static void _MULTIEDIT_Paint(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ * pObj) {
  I32 ScrollPosX, ScrollPosY, EffectSize, HBorder, IsRTL = 0;
  I32 x, y, xOff, yOff, ColorIndex, FontSizeY;
  GUI_RECT r, rClip;
  ...
  if (pObj->hText) {
    const char* pText;
    I32 Line = 0;
    I32 xSize       = _GetXSize(hObj, pObj);
    I32 NumVisLines = _GetNumVisLines(hObj, pObj);
    /* Get the text */
    pText = (const char *)GUI_ALLOC_h2p(pObj->hText);
...
*          0 else
*/
static I32 _AddKey(MULTIEDIT_HANDLE hObj, U16 Key) {
  I32 r = 0;               /* Key has not been consumed */
  MULTIEDIT_OBJ* pObj;
  pObj = MULTIEDIT_H2P(hObj);
  switch (Key) {
  case GUI_KEY_UP:
...
static void _SetText(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ * pObj, const char * pNew) {
  I32 NumCharsNew, NumCharsOld, NumBytesNew, NumBytesOld;
  char * pText;
    ...
    if (!pObj->hText) {
      ...
      if (NumCharsNew > 0) {
        ...
        if (_IsSpaceInBuffer(pObj, NumBytesNew)) {
          pText = (char *)GUI_ALLOC_h2p(pObj->hText);
          GUI_memmove(pText + pObj->CursorPosByte + NumBytesNew, 
                  pText + pObj->CursorPosByte, 
...
*       MULTIEDIT_GetText
*/
void MULTIEDIT_GetText(MULTIEDIT_HANDLE hObj, char* sDest, I32 MaxLen) {
  if (hObj) {
    MULTIEDIT_OBJ* pObj;
    WM_LOCK();
    pObj = MULTIEDIT_H2P(hObj);
...
*       MULTIEDIT_GetPrompt
*/
void MULTIEDIT_GetPrompt(MULTIEDIT_HANDLE hObj, char * sDest, I32 MaxLen) {
  if (hObj) {
    MULTIEDIT_OBJ * pObj;
    WM_LOCK();
    pObj = MULTIEDIT_H2P(hObj);
    if (pObj) {
      ...
      I32 Len = GUI_UC__NumChars2NumBytes(sSource, pObj->NumCharsPrompt);
      if (Len > (MaxLen - 1)) {
        Len = MaxLen - 1;
      }
...
void MULTIEDIT_SetPrompt(MULTIEDIT_HANDLE hObj, const char* pPrompt) {
  if (hObj) {
    MULTIEDIT_OBJ* pObj;
    I32 NumCharsNew = 0, NumCharsOld = 0;
    I32 NumBytesNew = 0, NumBytesOld = 0;
    char* pText;
    WM_LOCK();
    pObj = MULTIEDIT_H2P(hObj);
    if (pObj->hText) {
    ...
    if (_IsCharsAvailable(pObj, NumCharsNew - NumCharsOld)) {
      if (_IsSpaceInBuffer(pObj, NumBytesNew - NumBytesOld)) {
        pText = (char*) GUI_ALLOC_h2p(pObj->hText);
        GUI_memmove(pText + NumBytesNew, pText + NumBytesOld, GUI_strlen(pText + NumBytesOld) + 1);
        if (pPrompt) {
