...
void GUI_DispString(const char GUI_UNI_PTR *s)
{
	...
	for (; *s; s++) {
		GUI_RECT r;
		I32 LineNumChars = GUI__GetLineNumChars(s, 0x7fff);
		I32 xLineSize    = GUI__GetLineDistX(s, LineNumChars);
		/* Check if x-position needs to be changed due to h-alignment */
		switch (GUI_Context.TextAlign & GUI_TA_HORIZONTAL) {
			case GUI_TA_CENTER: xAdjust = xLineSize / 2;
			break;
			case GUI_TA_RIGHT:  xAdjust = xLineSize;
			break;
			default:            xAdjust = 0;
			break;
	}
