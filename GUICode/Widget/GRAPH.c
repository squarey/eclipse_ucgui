...
GRAPH_PROPS GRAPH__DefaultProps = {
  .aColor[0] = GRAPH_BKCOLOR_DEFAULT,
  .aColor[1] = GRAPH_BORDERCOLOR_DEFAULT,
  .aColor[2] = GRAPH_FRAMECOLOR_DEFAULT,
  .aColor[3] = GRAPH_GRIDCOLOR_DEFAULT,
  .GridSpacingX = GRAPH_GRIDSPACING_X_DEFAULT,
  .GridSpacingY = GRAPH_GRIDSPACING_Y_DEFAULT,
  .GridOffX = 0,
  .GridOffY = 0,
  .BorderL = GRAPH_BORDER_L_DEFAULT,
  .BorderT = GRAPH_BORDER_T_DEFAULT,
  .BorderR = GRAPH_BORDER_R_DEFAULT,
  .BorderB = GRAPH_BORDER_B_DEFAULT,
};
...
static void _DrawGrid(GRAPH_OBJ * pObj, GRAPH_Handle hObj, unsigned BorderL, unsigned BorderT, unsigned BorderR, unsigned BorderB) {
  GUI_RECT Rect;
  U8 LineStyleOld;
  I32 i, xSize, ySize, xSpace, ySpace, EffectSize;
...
static void _DrawBorder(GRAPH_OBJ * pObj, GRAPH_Handle hObj, unsigned BorderL, unsigned BorderT, unsigned BorderR, unsigned BorderB, GUI_RECT * pRectInvalid) {
  if (BorderL || BorderT || BorderR || BorderB) {
    I32 OrgX, OrgY, x0, y0, x1, y1, x0_Clear, y0_Clear, x1_Clear, y1_Clear;
    GUI_RECT Rect;
    I32 EffectSize;
    EffectSize = pObj->Widget.pEffect->EffectSize;
    WM_GetInsideRectExScrollbar(hObj, &Rect);
...
static void _ManageScrollBars(GRAPH_OBJ * pObj, GRAPH_Handle hObj) {
  I32 IsRequired, PageSize;
  GUI_RECT Rect;
  unsigned BorderL, BorderT, BorderR, BorderB, EffectSize;
  WM_GetInsideRectExScrollbar(hObj, &Rect);
  _GetParams(pObj, &BorderL, &BorderT, &BorderR, &BorderB, &EffectSize);
  PageSize   = Rect.y1 - Rect.y0 - BorderT - BorderB;
  IsRequired = ((I32)pObj->RangeY > PageSize) ? 1 : 0;
  ...
  IsRequired = ((I32)pObj->RangeX > PageSize) ? 1 : 0;
...
*       GRAPH__AddValue
*/
void GRAPH__AddValue(GRAPH_DATA_OBJ * pDataObj, void * pData, void * pValue, I32 Size) {
  if (pDataObj->PaintObj.hGraph) {
    if (pDataObj->NumItems == pDataObj->MaxNumItems) {
      GUI_memmove((U8 *)pData, (U8 *)pData + Size, Size * (pDataObj->MaxNumItems - 1));
      GUI_memmove((U8 *)pData + Size * (pDataObj->NumItems - 1), pValue, Size);
    } else {
      GUI_memmove((U8 *)pData + Size * pDataObj->NumItems++, pValue, Size);
    }
...
*       GRAPH_CreateEx
*/
GRAPH_Handle GRAPH_CreateEx(I32 x0, I32 y0, I32 xsize, I32 ysize, WM_HWIN hParent,
...
*       GRAPH_SetUserDraw
*/
void GRAPH_SetUserDraw(GRAPH_Handle hObj, void (* pUserDraw)(WM_HWIN hObj, I32 Stage)) {
  if (hObj) {
    GRAPH_OBJ * pObj;
    WM_LOCK();
    pObj = GRAPH_H2P(hObj);
