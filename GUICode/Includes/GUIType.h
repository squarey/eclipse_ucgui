...
typedef struct
{
	void  (* pfDraw)(I32 x0,I32 y0,I32 xsize, I32 ysize, const U8 GUI_UNI_PTR * pPixel, const LCD_LOGPALETTE GUI_UNI_PTR * pLogPal, I32 xMag, I32 yMag);
	GUI_COLOR (* pfIndex2Color)(I32 Index);
} GUI_BITMAP_METHODS;
...
***********************************************************************
*/
typedef U16  uctGUI_GetCharCode(const char GUI_UNI_PTR *s);
typedef I32  uctGUI_GetCharSize(const char GUI_UNI_PTR *s);
typedef I32  uctGUI_CalcSizeOfChar(U16 Char);
typedef I32  uctGUI_Encode(char *s, U16 Char);

typedef struct
{
	uctGUI_GetCharCode*            pfGetCharCode;
	uctGUI_GetCharSize*            pfGetCharSize;
	uctGUI_CalcSizeOfChar*         pfCalcSizeOfChar;
	uctGUI_Encode*                 pfEncode;
} GUI_UC_ENC_APILIST;
...
typedef I32  uctGUI_GetLineDistX(const char GUI_UNI_PTR *s, I32 Len);
typedef I32  uctGUI_GetLineLen(const char GUI_UNI_PTR *s, I32 MaxLen);
typedef void tGL_DispLine(const char GUI_UNI_PTR *s, I32 Len);

typedef struct
{
	uctGUI_GetLineDistX*          pfGetLineDistX;
	uctGUI_GetLineLen*            pfGetLineLen;
	tGL_DispLine*               pfDispLine;
} uctGUI_ENC_APIList;

extern const uctGUI_ENC_APIList GUI_ENC_APIList_SJIS;
...
typedef I32  GUI_GETCHARDISTX(U16P c);
...
#define ucDECLARE_FONT(Type)                                     \
void GUI##Type##_DispChar    (U16P c);                         \
I32  GUI##Type##_GetCharDistX(U16P c);                         \
...
struct GUI_FONT
{
	...
	const uctGUI_ENC_APIList* pafEncode;
	U8 YSize;
	U8 YDist;
	U8 XMag;
	U8 YMag;
...
typedef struct uctGUI_SIF_APIList_struct
{
...
#define GUI_SIF_TYPE          uctGUI_SIF_APIList
...
typedef U8 (*GUI_GetTouchData)(I32 *TouchX, I32 *TouchY, U8 *pState, U8 TouchID);

typedef struct
{
	LCD_Driver_t	tLCDDriver;
	GUI_GetTouchData tTouchDriver;
}GUI_Driver_t;
