...
/* Define colors */
  ...
  #define LISTBOX_BKCOLOR0_DEFAULT 0//GUI_WHITE     /* Not selected */
#endif

#ifndef LISTBOX_BKCOLOR1_DEFAULT
  #define LISTBOX_BKCOLOR1_DEFAULT 0//GUI_WHITE      /* Selected, no focus */
#endif
  ...
  #define LISTBOX_TEXTCOLOR0_DEFAULT GUI_WHITE   /* Not selected */
#endif
...
*       _CallOwnerDraw
*/
static I32 _CallOwnerDraw(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj, I32 Cmd, I32 ItemIndex) {
...
*       _GetYSize
*/
static I32 _GetYSize(LISTBOX_Handle hObj) {
  GUI_RECT Rect;
...
*       _GetItemSizeX
*/
static I32 _GetItemSizeX(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj, unsigned Index) {
  LISTBOX_ITEM* pItem;
  I32 xSize = 0;
...
*       _GetItemSizeY
*/
static I32 _GetItemSizeY(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj, unsigned Index) {
  LISTBOX_ITEM* pItem;
  I32 ySize = 0;
  pItem = (LISTBOX_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray, Index);
  if (pItem) {
...
*       _GetContentsSizeX
*/
static I32 _GetContentsSizeX(LISTBOX_Handle hObj) {
  LISTBOX_Obj* pObj;
  I32 i, NumItems, SizeX;
  I32 Result = 0;
...
*       _GetItemPosY
*/
static I32 _GetItemPosY(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj, unsigned Index) {
  if (Index < LISTBOX__GetNumItems(pObj)) {
    if ((I32)Index >= pObj->ScrollStateV.v) {
      unsigned i;
      I32 PosY = 0;
...
*       _IsPartiallyVis
*/
static I32 _IsPartiallyVis(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj) {
  I32 Index;
  Index = pObj->Sel;
  if (Index < (I32)LISTBOX__GetNumItems(pObj)) {
...
I32 LISTBOX_OwnerDraw(const WIDGET_ITEM_DRAW_INFO* pDrawItemInfo) {
  switch (pDrawItemInfo->Cmd) {
    ...
    case WIDGET_ITEM_DRAW: {
      ...
      GUI_RECT r, rText;
      I32 FontDistY;
      I32 ItemIndex = pDrawItemInfo->ItemIndex;
      const char* s;
      I32 ColorIndex;
      char IsDisabled;
      char IsSelected;
...
*/
static I32 _UpdateScrollPos(LISTBOX_Handle hObj, LISTBOX_Obj* pObj) {
  I32 PrevScrollStateV;
...
*       _CalcScrollParas
*/
static I32 _CalcScrollParas(LISTBOX_Handle hObj) {
  GUI_RECT Rect;
  LISTBOX_Obj* pObj = LISTBOX_H2P(hObj);
...
*       _IsAlphaNum
*/
static I32 _IsAlphaNum(I32 Key) {
  Key = _Tolower(Key);
  if (Key >= 'a' && Key <= 'z') {
    return 1;
  }
...
*       _SelectByKey
*/
static void _SelectByKey(LISTBOX_Handle hObj, I32 Key) {
  unsigned i;
  LISTBOX_Obj* pObj;
  pObj = LISTBOX_H2P(hObj);
  Key = _Tolower(Key);
...
static void _OnPaint(LISTBOX_Handle hObj, LISTBOX_Obj* pObj, WM_MESSAGE* pMsg) {
  ...
  for (i = pObj->ScrollStateV.v; i < NumItems; i++) {
    ...
    if (RectItem.y1 >= ClipRect.y0) {
	  ...
	  GUI_SetColor((~LISTBOX_BKCOLOR0_DEFAULT) | (0xff000000));
	  GUI_DrawHLine(RectItem.y1,RectItem.x0,RectItem.x1);
    }
...
*       _ToggleMultiSel
*/
static void _ToggleMultiSel(LISTBOX_Handle hObj, LISTBOX_Obj* pObj, I32 Sel) {
  if (pObj->Flags & LISTBOX_SF_MULTISEL) {
...
*       _GetItemFromPos
*/
static I32 _GetItemFromPos(LISTBOX_Handle hObj, LISTBOX_Obj* pObj, I32 x, I32 y) {
  I32 Sel = -1;
  GUI_RECT Rect;
  WM_GetInsideRectExScrollbar(hObj, &Rect);
  if ((x >= Rect.x0) && (y >= Rect.y0)) {
    if ((x <= Rect.x1) && (y <= Rect.y1)) {
      I32 NumItems = LISTBOX__GetNumItems(pObj);
      I32 i, y0 = Rect.y0;
      for (i = pObj->ScrollStateV.v; i < NumItems; i++) {
        if (y >= y0) {
          Sel = i;
...
*       _OnTouch
*/
static void _OnTouch(LISTBOX_Handle hObj, LISTBOX_Obj* pObj, WM_MESSAGE* pMsg) {
...
/* ��ӻ���listbox������  �Զ�����listbox */
static void _OnTouchMoveV(LISTBOX_Handle hObj,LISTBOX_Obj* pObj, WM_MESSAGE* pMsg)
{
	...
	I32 FirstVis = 0;
	I32 CurSelYSize = 0;
	const GUI_PID_STATE* tState = (const GUI_PID_STATE*)pMsg->Data.p;
	...
	{
		isFirstTouch = 1;
		lastTouchYPos = tState->y;
		return;
	}
	//��ȡһ��ҳ���������ʾ��item��
	VisItems = _GetNumVisItems(pObj, hObj);
	//��ȡlistbox���ܹ���item
	TotalItems = LISTBOX__GetNumItems(pObj);
	//�ж��Ƿ���Ҫ������ʾ
	IsRequired = (VisItems < TotalItems) ? 1 : 0;
	if(!IsRequired)
	{
		lastTouchYPos = 0;
		return;
	}
	max = TotalItems - VisItems;
	FirstVis = pObj->ScrollStateV.v;
	CurSelYSize = _GetItemSizeY(hObj,pObj,pObj->Sel);
	/* �����Y���ϻ����ľ������������ѡ��item��Y��С  �����»�������Ч */
	
	if((tState->y - lastTouchYPos) >= CurSelYSize)
	{
		FirstVis--;
		lastTouchYPos = tState->y;
		pObj->isMove = 1;
	...
	/* ���ϻ�������Ч */
	else if(tState->y  <= (lastTouchYPos - CurSelYSize))
	{
		FirstVis++;
		lastTouchYPos = tState->y;
		pObj->isMove = 1;
...
*/
static void _MoveSel(LISTBOX_Handle hObj, I32 Dir) {
  I32 Index, NewSel = -1, NumItems;
  LISTBOX_Obj * pObj;
  pObj = LISTBOX_H2P(hObj);
...
*          0 else 
*/
static I32 _AddKey(LISTBOX_Handle hObj, I32 Key) {
  LISTBOX_Obj* pObj;
  pObj = LISTBOX_H2P(hObj);
  switch (Key) {
  case GUI_KEY_SPACE:
...
*/
const char* LISTBOX__GetpString(const LISTBOX_Obj* pObj, I32 Index) {
  const char* s = NULL;
...
*/
void LISTBOX__InvalidateItem(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj, I32 Sel) {
  if (Sel >= 0) {
    I32 ItemPosY;
...
*/
void LISTBOX__InvalidateItemAndBelow(LISTBOX_Handle hObj, const LISTBOX_Obj* pObj, I32 Sel) {
  if (Sel >= 0) {
    I32 ItemPosY;
    ItemPosY = _GetItemPosY(hObj, pObj, Sel);
    if (ItemPosY >= 0) {
...
void LISTBOX_AddString(LISTBOX_Handle hObj, const char* s) {
  if (hObj && s) {
    ...
    WM_LOCK();
    pObj = LISTBOX_H2P(hObj);
    if (GUI_ARRAY_AddItem(&pObj->ItemArray, &Item, sizeof(LISTBOX_ITEM) + GUI_strlen(s)) == 0) {
...
*       LISTBOX_SetSel
*/
void LISTBOX_SetSel (LISTBOX_Handle hObj, I32 NewSel) {
  if (hObj) {
    LISTBOX_Obj* pObj;
    I32 MaxSel;
    WM_LOCK();
...
I32 LISTBOX_GetSelWithString(LISTBOX_Handle hObj, char *str)
{
    ...
    pObj = LISTBOX_H2P(hObj);
    r = pObj->Sel;
	pItem= (LISTBOX_ITEM*)GUI_ARRAY_GetpItem(&pObj->ItemArray, r);
	GUI_strcpy(str,pItem->acText);
    WM_UNLOCK();
  }
  return r;
}
