...
*       _Value2Pos
*/
static I32 _Value2Pos(const PROGBAR_Obj* pObj, I32 v) {
  I32 IsVertical = pObj->Flags & PROGBAR_CF_VERTICAL;
  I32 EffectSize = pObj->Widget.pEffect->EffectSize;
  I32 Size  = IsVertical ? pObj->Widget.Win.Rect.y1 - pObj->Widget.Win.Rect.y0 + 1 : pObj->Widget.Win.Rect.x1 - pObj->Widget.Win.Rect.x0 + 1;
  I32 Min   = pObj->Min;
  I32 Max   = pObj->Max;
  if (v < Min) {
	  v = Min;
...
*       _DrawPart
*/
static void _DrawPart(const PROGBAR_Obj* pObj, I32 Index,
										  I32 xText, I32 yText, const char* pText) {
...
static void _Paint(PROGBAR_Obj * pObj) {
  ...
  if (pObj->Flags & PROGBAR_CF_VERTICAL) {
    ...
    pText = PROGBAR_GetText(pObj, ac);
    GUI_SetFont(pObj->Props.pFont);
    PROGBAR_GetTextRect(pObj, &rText, pText);
...
*       PROGBAR_GetTextRect
*/
void PROGBAR_GetTextRect(const PROGBAR_Obj* pObj, GUI_RECT* pRect, const char* pText) {
  I32 xSize      = pObj->Widget.Win.Rect.x1 - pObj->Widget.Win.Rect.x0 + 1;
  I32 ySize      = pObj->Widget.Win.Rect.y1 - pObj->Widget.Win.Rect.y0 + 1;
  I32 TextWidth  = GUI_GetStringDistX(pText);
  I32 TextHeight = GUI_GetFontSizeY();
  I32 EffectSize = pObj->Widget.pEffect->EffectSize;
    ...
    if (pObj->v != v) {
      ...
      if (pObj->Flags & PROGBAR_CF_VERTICAL) {
        ...
        if (pObj->hpText == 0) {
          ...
          PROGBAR_GetTextRect(pObj, &rText, PROGBAR_GetText(pObj, acBuffer));
