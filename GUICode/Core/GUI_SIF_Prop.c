...
*       _GetNumCharAreas
*/
static I32 _GetNumCharAreas(const GUI_FONT GUI_UNI_PTR * pFont) {
  U16 NumCharAreas;
  const U8 * pData;
...
static void _GUI_SIF_DispChar(U16P c) {
  ...
  if (pCharInfo) {
    ...
    if (GUI_Context.pAFont->YDist > GUI_Context.pAFont->YSize) {
      I32 YMag = GUI_Context.pAFont->YMag;
      I32 YDist = GUI_Context.pAFont->YDist * YMag;
      I32 YSize = GUI_Context.pAFont->YSize * YMag;
...
*/
static I32 _GUI_SIF_GetCharDistX(U16P c) {
  const U8 * pCharInfo;
  U16 DistX = 0;
...
*       GUI_SIF_TYPE_PROP
*/
const uctGUI_SIF_APIList GUI_SIF_APIList_Prop = {
  _GUI_SIF_DispChar,
