...
*       _GetLineDistX_SJIS
*/
static I32 _GetLineDistX_SJIS(const char GUI_UNI_PTR *s, I32 Len) {
  I32 Dist =0;
  if (s) {
    U8 c0;
...
*/
static I32 _GetLineLen_SJIS(const char GUI_UNI_PTR *s, I32 MaxLen) {
  I32 Len =0;
  U8 c0;
...
*       _DispLine_SJIS
*/
static void _DispLine_SJIS(const char GUI_UNI_PTR *s, I32 Len) {
  U8 c0;
  while (--Len >=0) {
    c0=*(const U8*)s++;
    if (c0 > 127) {
...
*/
const uctGUI_ENC_APIList GUI_ENC_APIList_SJIS = {
  _GetLineDistX_SJIS,
  _GetLineLen_SJIS,
  _DispLine_SJIS
};
