...
*       _BinChar2Int
*/
static I32 _BinChar2Int(I32 Char) {
  if ((Char >= '0') && (Char <= '1'))
    return Char - '0';
  return -1;
}
...
static void _EditBin(U8 Bit, EDIT_Obj* pObj, EDIT_Handle hObj) {
  I32 Pos = pObj->MaxLen - pObj->CursorPos - 1;   /* Bit position */
  U32 AndMask = ~(1   << Pos);
  U32 OrMask  =   Bit << Pos;
...
*       _AddKeyBin
*/
static void _AddKeyBin(EDIT_Handle hObj, I32 Key) {
  EDIT_Obj * pObj;
  pObj = EDIT_H2P(hObj); /* The GUI needs not to be locked here. This function is called only from EDIT_AddKey which has already locked the GUI */
  if (pObj) {
    switch (Key) {
