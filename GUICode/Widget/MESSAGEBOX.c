...
#if GUI_WINSUPPORT && MESSAGEBOX_SUPPORT
...
void MESSAGEBOX_Callback(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
    ...
    case WM_NOTIFY_PARENT:
      {
        I32 NCode = pMsg->Data.v;             /* Get notification code */
        I32 Id    = WM_GetId(pMsg->hWinSrc);  /* Get control ID */
        switch (NCode) {
...
*       MESSAGEBOX_Create
*/
WM_HWIN MESSAGEBOX_Create(const char * sMessage, const char * sCaption, I32 Flags) {
  GUI_WIDGET_CREATE_INFO _aDialogCreate[3];                                     /* 0: FrameWin, 1: Text, 2: Button */
  I32 BorderSize = FRAMEWIN_GetDefaultBorderSize();                             /* Default border size of frame window */
  I32 xSizeFrame = MESSAGEBOX_XSIZEOK + 2 * BorderSize + MESSAGEBOX_BORDER * 2; /* XSize of frame window */
  I32 ySizeFrame;                                                               /* YSize of frame window */
  I32 x0, y0;                                                                   /* Position of frame window */
  I32 xSizeMessage;                                                             /* Length in pixels of message */
  I32 xSizeCaption;                                                             /* Length in pixels of caption */
  I32 ySizeCaption;                                                             /* YSize of caption */
  I32 ySizeMessage;                                                             /* YSize of message */
  WM_HWIN hBox;
  GUI_RECT Rect;
  const GUI_FONT GUI_UNI_PTR * pOldFont;
  /* Zeroinit variables */
  GUI_memset(_aDialogCreate, 0, sizeof(_aDialogCreate));
