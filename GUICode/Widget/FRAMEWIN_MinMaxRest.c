...
*       _InvalidateButton
*/
static void _InvalidateButton(FRAMEWIN_Obj* pObj, I32 Id) {
  WM_HWIN hChild;
  WM_Obj* pChild;
...
static void _RestoreMinimized(FRAMEWIN_Handle hObj, FRAMEWIN_Obj* pObj) {
  ...
  if (pObj->Flags & FRAMEWIN_SF_MINIMIZED) {
    I32 OldHeight = 1 + pObj->Widget.Win.Rect.y1 - pObj->Widget.Win.Rect.y0;
    I32 NewHeight = 1 + pObj->rRestore.y1 - pObj->rRestore.y0;
...
static void _MinimizeFramewin(FRAMEWIN_Handle hObj, FRAMEWIN_Obj* pObj) {
  ...
  if ((pObj->Flags & FRAMEWIN_SF_MINIMIZED) == 0) {
    I32 OldHeight = pObj->Widget.Win.Rect.y1 - pObj->Widget.Win.Rect.y0 + 1;
    I32 NewHeight = FRAMEWIN__CalcTitleHeight(pObj) + pObj->Widget.pEffect->EffectSize * 2 + 2;    
