...
*       _AddScrollbar
*/
static void _AddScrollbar(MULTIPAGE_Handle hObj, MULTIPAGE_Obj* pObj, I32 x, I32 y, I32 w, I32 h) {
...
*       _SetScrollbar
*/
static void _SetScrollbar(MULTIPAGE_Handle hObj, MULTIPAGE_Obj* pObj, I32 NumItems) {
  SCROLLBAR_Handle hScroll;
...
static void _ShowPage(MULTIPAGE_Obj* pObj, unsigned Index) {
  ...
  if ((I32)Index < pObj->Handles.NumItems) {
...
*       _SetEnable
*/
static void _SetEnable(MULTIPAGE_Obj* pObj, unsigned Index, I32 State) {
  if ((I32)Index < pObj->Handles.NumItems) {
    MULTIPAGE_PAGE* pPage;
    pPage = (MULTIPAGE_PAGE*) GUI_ARRAY_GetpItem(&pObj->Handles, Index);
    if (State) {
...
*       _GetEnable
*/
static I32 _GetEnable(MULTIPAGE_Obj* pObj, unsigned Index) {
  I32 r = 0;
  if ((I32)Index < pObj->Handles.NumItems) {
    MULTIPAGE_PAGE* pPage;
    pPage = (MULTIPAGE_PAGE*) GUI_ARRAY_GetpItem(&pObj->Handles, Index);
...
*/
static I32 _GetPageSizeX(MULTIPAGE_Obj* pObj, unsigned Index) {
  I32 r = 0;
  if ((I32)Index < pObj->Handles.NumItems) {
    MULTIPAGE_PAGE* pPage;
...
*/
static I32 _GetPagePosX(MULTIPAGE_Obj* pObj, unsigned Index) {
  unsigned i, r = 0;
  for (i = 0; i < Index; i++) {
...
*/
static I32 _GetTextWidth(MULTIPAGE_Obj* pObj) {
...
static void _DrawTextItem(MULTIPAGE_Obj* pObj, const char* pText, unsigned Index,
                          const GUI_RECT* pRect, I32 x0, I32 w, I32 ColorIndex) {
  GUI_RECT r;
  r = *pRect;
  r.x0 += x0;
  r.x1  = r.x0 + w;
...
*       _ClickedOnMultipage
*/
static I32 _ClickedOnMultipage(MULTIPAGE_Handle hObj, MULTIPAGE_Obj* pObj, I32 x, I32 y) {
  GUI_RECT rText;
  _GetTextRect(pObj, &rText);
  if ((y >= rText.y0) && (y <= rText.y1)) {
    if ((pObj->Handles.NumItems > 0) && (x >= rText.x0) && (x <= rText.x1)) {
      I32 i, w = 0, x0 = rText.x0;
...
*/
static void _MoveSel(MULTIPAGE_Handle hObj, MULTIPAGE_Obj * pObj, I32 Dir) {
  I32 Sel, NewSel, ScrollPos, NumItems;
  SCROLLBAR_Handle hScroll;
  hScroll  = WM_GetScrollbarH(hObj);
  NewSel   = ScrollPos = -1;
...
*          0 else 
*/
static I32 _AddKey(MULTIPAGE_Handle hObj, MULTIPAGE_Obj * pObj, I32 Key) {
  switch (Key) {
  case GUI_KEY_PGUP:
...
*       MULTIPAGE_CreateEx
*/
MULTIPAGE_Handle MULTIPAGE_CreateEx(I32 x0, I32 y0, I32 xsize, I32 ysize, WM_HWIN hParent,
...
void MULTIPAGE_AddPage(MULTIPAGE_Handle hObj, WM_HWIN hWin ,const char* pText) {
  ...
  if (hObj) {
    ...
    if (hWin) {
      ...
      if (GUI_ARRAY_AddItem(&pObj->Handles, &Page, sizeof(MULTIPAGE_PAGE) + GUI_strlen(pText)) == 0) {
        MULTIPAGE_PAGE* pPage;
        pPage = (MULTIPAGE_PAGE*) GUI_ARRAY_GetpItem(&pObj->Handles, pObj->Handles.NumItems - 1);
        GUI_memcpy(&pPage->acText, pText, GUI_strlen(pText) + 1);
      }
...
*/
void MULTIPAGE_DeletePage(MULTIPAGE_Handle hObj, unsigned Index, I32 Delete) {
  if (hObj) {
    MULTIPAGE_Obj* pObj;
    WM_LOCK();
    pObj = MULTIPAGE_H2P(hObj);
  ...
  if (hObj && pText) {
    ...
    if (pObj) {
      if ((I32)Index < pObj->Handles.NumItems) {
        ...
        if (GUI_ARRAY_SetItem(&pObj->Handles, Index, &Page, sizeof(MULTIPAGE_PAGE) + GUI_strlen(pText))) {
          pPage = (MULTIPAGE_PAGE*) GUI_ARRAY_GetpItem(&pObj->Handles, Index);
          GUI_memcpy(&pPage->acText, pText, GUI_strlen(pText) + 1);          
          _UpdatePositions(hObj, pObj);
        }
...
void MULTIPAGE_SetBkColor(MULTIPAGE_Handle hObj, GUI_COLOR Color, unsigned Index) {
  MULTIPAGE_Obj* pObj;
  if (hObj && ((I32)Index < MULTIPAGE_NUMCOLORS)) {
    WM_LOCK();
    pObj = MULTIPAGE_H2P(hObj);
    if (pObj) {
...
*/
I32 MULTIPAGE_GetSelection(MULTIPAGE_Handle hObj) {
  I32 r = 0;
  if (hObj) {
    MULTIPAGE_Obj* pObj;
    WM_LOCK();
...
*/
I32 MULTIPAGE_IsPageEnabled(MULTIPAGE_Handle hObj, unsigned Index) {
  I32 r = 0;
  if (hObj) {
    MULTIPAGE_Obj* pObj;
    WM_LOCK();
