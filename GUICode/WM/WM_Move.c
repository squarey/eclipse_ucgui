...
*/
static void _MoveDescendents(WM_HWIN hWin, I32 dx, I32 dy) {
  WM_Obj* pWin;
...
void WM__MoveWindow(WM_HWIN hWin, I32 dx, I32 dy)
{
	...
	if ((hWin) && (dx || dy)) {
		...
		if (pWin->Status & WM_SF_ISVIS) {
			/* Invalidate old area */
			WM__InvalidateRectEx(&r, pWin->hParent);
			/* Invalidate new area */
		...
		if (pWin->Status & WM_SF_ISVIS) {
			GUI_RectIntersectLCD(&r, &pWin->Rect);
			WM__InvalidateRectEx(&r, pWin->hParent); /* Invalidate new area */
		}
#endif
