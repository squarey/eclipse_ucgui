...
void GUIMONO_DispChar(U16P c) {
  ...
  if (c0!=-1) {
    I32 BytesPerChar = GUI_Context.pAFont->YSize*pMono->BytesPerLine;
    GUI_DRAWMODE DrawMode;
    I32 XSize = pMono->XSize;
    I32 YSize = GUI_Context.pAFont->YSize;
...
*/
I32 GUIMONO_GetCharDistX(U16P c) {
  const GUI_FONT_MONO GUI_UNI_PTR * pMono = GUI_Context.pAFont->p.pMono;
  GUI_USE_PARA(c);
  return pMono->XDist;
}
