...
*       _SetActive
*/
static void _SetActive(FRAMEWIN_Handle hObj, I32 State) {
  FRAMEWIN_Obj* pObj;
...
static void _Paint(FRAMEWIN_Obj* pObj) {
  ...
  I32 xsize = WM_GetWindowSizeX(hWin);
  I32 ysize = WM_GetWindowSizeY(hWin);
  I32 BorderSize = pObj->Props.BorderSize;
  I32 y0, Index;
  POSITIONS Pos;
  GUI_RECT r, rText;
...
static void _OnChildHasFocus(FRAMEWIN_Handle hWin, FRAMEWIN_Obj* pObj, WM_MESSAGE *pMsg) {
  if (pMsg->Data.p) {
    ...
    I32 IsDesc = WM__IsAncestorOrSelf(pInfo->hNew, hWin);
...
*/
I32 FRAMEWIN__CalcTitleHeight(FRAMEWIN_Obj* pObj) {
  I32 r = 0;
