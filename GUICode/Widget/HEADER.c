...
static const GUI_UNI_PTR * _pOldCursor;

/* Default values */
static const GUI_UNI_PTR * _pDefaultCursor   = HEADER_CURSOR_DEFAULT;
static GUI_COLOR                      _DefaultBkColor   = HEADER_BKCOLOR_DEFAULT;
static GUI_COLOR                      _DefaultTextColor = HEADER_TEXTCOLOR_DEFAULT;
static I32                            _DefaultBorderH   = HEADER_BORDER_H_DEFAULT;
static I32                            _DefaultBorderV   = HEADER_BORDER_V_DEFAULT;
...
*       _DrawTriangle
*/
static void _DrawTriangle(I32 x, I32 y, I32 Size, I32 Inc) {
...
static void _Paint(HEADER_Obj* pObj) {
  ...
  I32 i, xPos = -pObj->ScrollPos;
  I32 NumItems = GUI_ARRAY_GetNumItems(&pObj->Columns);
  I32 EffectSize = pObj->Widget.pEffect->EffectSize;
  I32 ArrowSize, ArrowPos;
...
static I32 _GetDividerIndex(HEADER_Handle hObj, HEADER_Obj * pObj, I32 x, I32 y) {
  I32 Item = -1;
  if ((y >= 0) && (y < WM_GetWindowSizeY(hObj))) {
    if (hObj) {
      I32 Index, xPos = 0, NumColumns;
...
*/
static I32 _LimitDragWitdh(HEADER_Handle hObj, HEADER_Obj * pObj) {
  if (pObj->DragLimit) {
    I32 DragLimit, i, SumX;
    GUI_RECT Rect;
...
*/
#if (HEADER_SUPPORT_DRAG)
static void _HandlePID(HEADER_Handle hObj, HEADER_Obj * pObj, I32 x, I32 y, I32 Pressed) {
  I32 Hit = _GetDividerIndex(hObj, pObj, x, y);
  ...
  if ((pObj->CapturePosX >= 0) && (x != pObj->CapturePosX) && (Pressed == 1)) {
    I32 NewSize = HEADER_GetItemWidth(hObj, pObj->CaptureItem) + x - pObj->CapturePosX;
    if (NewSize >= 0) {
...
*       _GetItemIndex
*/
static I32 _GetItemIndex(HEADER_Handle hObj, HEADER_Obj * pObj, I32 x, I32 y) {
  I32 Item = -1;
  if ((y >= 0) && (y < WM_GetWindowSizeY(hObj))) {
    if (hObj) {
      I32 Index, xPos = 0, NumColumns;
...
*       HEADER_Create
*/
HEADER_Handle HEADER_Create(I32 x0, I32 y0, I32 xsize, I32 ysize, WM_HWIN hParent, I32 Id, I32 Flags, I32 ExFlags) {
...
*/
const  GUI_UNI_PTR * HEADER_SetDefaultCursor(const GUI_CURSOR * pCursor) {
  const  GUI_UNI_PTR * pOldCursor = _pDefaultCursor;
...
I32 HEADER_SetDefaultBorderH(I32 Spacing) {
  I32 OldSpacing = _DefaultBorderH;
...
I32 HEADER_SetDefaultBorderV(I32 Spacing) {
  I32 OldSpacing = _DefaultBorderV;
...
*       HEADER_GetDefault...
*/
const GUI_UNI_PTR *  HEADER_GetDefaultCursor(void)    { return _pDefaultCursor; }
...
I32                HEADER_GetDefaultBorderH(void)   { return _DefaultBorderH; }
I32                HEADER_GetDefaultBorderV(void)   { return _DefaultBorderV; }
...
void HEADER_AddItem(HEADER_Handle hObj, I32 Width, const char * s, I32 Align) {
  if (hObj) {
    ...
    if (GUI_ARRAY_AddItem(&pObj->Columns, &Column, sizeof(HEADER_COLUMN) + GUI_strlen(s) + 1) == 0) {
      HEADER_COLUMN * pColumn;
      pObj = HEADER_H2P(hObj);
      pColumn = (HEADER_COLUMN *)GUI_ARRAY_GetpItem(&pObj->Columns, Index);
      GUI_strcpy(pColumn->acText, s);
      WM_InvalidateWindow(hObj);
      WM_InvalidateWindow(WM_GetParent(hObj));
    }
    WM_UNLOCK();
...
void HEADER_SetItemText(HEADER_Handle hObj, U32 Index, const char* s) {
  if (hObj) {
    ...
    if (Index < GUI_ARRAY_GetNumItems(&pObj->Columns)) {
      HEADER_COLUMN* pColumn;
      pColumn = (HEADER_COLUMN*)GUI_ARRAY_ResizeItem(&pObj->Columns, Index, sizeof(HEADER_COLUMN) + GUI_strlen(s));
      if (pColumn) {
        GUI_strcpy(pColumn->acText, s);
      }
    }
...
*/
void HEADER_SetDirIndicator(HEADER_Handle hObj, I32 Column, I32 Reverse) {
  if (hObj) {
    HEADER_Obj * pObj;
    WM_LOCK();
    pObj = HEADER_H2P(hObj);
