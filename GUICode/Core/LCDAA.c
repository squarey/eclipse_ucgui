...
void LCD_SetPixelAA(I32 x, I32 y, U8 Intens)
{
	...
	RETURN_IF_X_OUT();
	if (Intens >= 15) {
		LCDDEV_L0_SetPixelIndex(x,y, LCD_COLORINDEX, 0xff);
	} else {
		...
		LCDDEV_L0_SetPixelIndex(x,y, LCD_Color2Index(Color), 0xff);
	}
}
...
void LCD_SetPixelAA_NoTrans(I32 x, I32 y, U8 Intens)
{
	...
	RETURN_IF_X_OUT();
	if (Intens == 0) {
		LCDDEV_L0_SetPixelIndex(x,y, LCD_BKCOLORINDEX, 0xff);
	} else if (Intens == 15) {
		LCDDEV_L0_SetPixelIndex(x,y, LCD_COLORINDEX, 0xff);
	} else {
